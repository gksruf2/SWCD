...
class ActNorm(nn.Module):
    def __init__(self, num_features, logdet=False, affine=True):
        ...
        # register_buffer : backpropagatin이 진행되지 않는(업데이트 되지 않는) 일반 layer
        self.register_buffer('initialized', torch.tensor(0, dtype=torch.uint8))

    def initialize(self, input):
        with torch.no_grad():
            # input (v) shape : (BS, 64, 1, 1)
            # contiguous : 메모리를 새롭게 할당하여, permute 등 tensor의 shape을 변경시킬 때 에러를 예방한다.
            # flatten shape : (BS, 64, 1, 1) -> (64, BS, 1, 1) -> (64, BS)
    ...
    def forward(self, input, reverse=False):
        ...
        if self.initialized.item() == 0:    # 초기상황
